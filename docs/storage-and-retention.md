...
5. Set the repository Actions secret `CRON_SECRET` to the same value. `.github/workflows/cleanup-expired-files.yml` calls `GET /api/cleanup-expired-files` hourly with the required Bearer token. The committed daily Vercel schedule remains a fallback for Vercel Hobby, which rejects hourly cron expressions. After upgrading to Vercel Pro or Enterprise, the GitHub schedule can instead be removed and `vercel.json` changed to an hourly cron.
...
Access is rejected immediately once `expires_at` reaches exactly 48 hours. The GitHub Actions scheduler normally performs physical deletion within one hour. The committed Vercel Hobby fallback runs daily if GitHub Actions is delayed or unavailable.
